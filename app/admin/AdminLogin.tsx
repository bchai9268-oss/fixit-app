...
type AdminLoginProps = { error?: string; setupComplete: boolean; configured: boolean; adminEmail?: string };
...
export default function AdminLogin({ error, setupComplete, configured, adminEmail = "" }: AdminLoginProps) {
  return (
    <main className="login-page">
      ...
      <section className="login-panel"><div className="login-box">
        ...
        {!configured ? <div className="auth-message notice"><strong>ยังไม่ได้สร้างบัญชีแอดมิน</strong><br />เปิดลิงก์ตั้งค่าครั้งแรกและสร้างรหัสผ่านก่อนเข้าสู่ระบบ</div> : <form action="/api/admin/login" method="post"><label>อีเมลแอดมิน<input required readOnly={Boolean(adminEmail)} type="email" name="email" defaultValue={adminEmail} autoComplete="username" placeholder="admin@example.com" /></label><label>รหัสผ่าน<div className="password-field"><input required type="password" name="password" minLength={12} autoComplete="current-password" placeholder="กรอกรหัสผ่านแอดมิน" /></div></label><button className="button button-primary full" type="submit">เข้าสู่ระบบ →</button></form>}
