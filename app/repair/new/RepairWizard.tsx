...
const brands: Record<DeviceKey, string[]> = {
  phone: ["Apple", "Samsung", "OPPO", "vivo", "Xiaomi", "realme", "Huawei", "HONOR"],
  laptop: ["Apple", "ASUS", "Acer", "Lenovo", "HP", "Dell", "MSI"],
  desktop: ["Custom Build", "ASUS", "Acer", "Lenovo", "HP", "Dell"],
  other: ["Apple", "Samsung", "Huawei", "Sony", "Nintendo", "Canon", "HP"],
};
...
const copy = {
  th: {
    ...
    step: (value: number) => `ขั้นตอนที่ ${value} จาก 3`, symptomTitle: "เครื่องของคุณมีอาการอย่างไรบ้าง?", symptomHelp: "เลือกประเมินเบื้องต้นได้มากกว่า 1 ข้อ", deviceLabel: "ประเภทอุปกรณ์", customDevice: "โปรดระบุว่าเป็นอุปกรณ์อะไร", customDeviceExample: "เช่น แท็บเล็ต เครื่องเล่นเกม เครื่องพิมพ์", next: "ถัดไป",
    contactTitle: "ข้อมูลอุปกรณ์และผู้ติดต่อ", brand: "ยี่ห้ออุปกรณ์", chooseBrand: "เลือกยี่ห้อ", otherBrand: "ยี่ห้ออื่น", otherBrandLabel: "โปรดระบุยี่ห้อ", model: "รุ่นอุปกรณ์", modelExample: "เช่น iPhone 13 Pro", name: "ชื่อผู้ติดต่อ", phone: "เบอร์โทรศัพท์", email: "อีเมล (ถ้ามี)", note: "รายละเอียดเพิ่มเติม", noteHelp: "ข้อมูลที่ช่วยให้ช่างตรวจสอบได้เร็วขึ้น", priority: "ความเร่งด่วน", normal: "ปกติ", normalHelp: "ดำเนินการตามลำดับคิว", urgent: "เร่งด่วน", urgentHelp: "ร้านจะติดต่อเพื่อยืนยันคิวและค่าบริการเพิ่มเติม", priorityNotice: "การเลือกเร่งด่วนยังไม่ใช่การยืนยันว่าจะซ่อมทันที กรุณารอร้านติดต่อกลับ", back: "ย้อนกลับ", review: "ตรวจสอบข้อมูล",
  ...
  en: {
    ...
    step: (value: number) => `Step ${value} of 3`, symptomTitle: "What is wrong with your device?", symptomHelp: "Select one or more initial symptoms", deviceLabel: "Device type", customDevice: "Specify the device type", customDeviceExample: "For example, tablet, game console, or printer", next: "Next",
    contactTitle: "Device and contact details", brand: "Device brand", chooseBrand: "Choose a brand", otherBrand: "Other brand", otherBrandLabel: "Specify the brand", model: "Device model", modelExample: "For example, iPhone 13 Pro", name: "Contact name", phone: "Phone number", email: "Email (optional)", note: "Additional details", noteHelp: "Information that may help our technician inspect it faster", priority: "Priority", normal: "Standard", normalHelp: "Handled in queue order", urgent: "Urgent", urgentHelp: "The shop will contact you to confirm the queue and any extra fee", priorityNotice: "Urgent selection does not guarantee immediate service. Please wait for confirmation from the shop.", back: "Back", review: "Review details",
...
export default function RepairWizard({ initialDevice }: { initialDevice: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const resolvedBrand = brand === "other" ? otherBrand.trim() : brand;
      const resolvedType = deviceType === "other" ? customDevice.trim() : devices[deviceType].th;
      const response = await fetch("/api/repairs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, phone, email, deviceType: resolvedType, deviceModel: `${resolvedBrand} ${model.trim()}`.trim(), symptoms: selectedSymptoms, note, priority }) });
  ...
  const resolvedBrand = brand === "other" ? otherBrand.trim() : brand;
  const resolvedTypeLabel = deviceType === "other" ? customDevice.trim() : devices[deviceType][language];
  const deviceDetailsComplete = Boolean(model.trim() && brand && (brand !== "other" || otherBrand.trim()));
  ...
  return (
    <main className="flow-page">
      ...
      <form className="wizard-card" onSubmit={submit}>
        {step === 1 && <section><p className="wizard-overline">{text.step(1)}</p><h1>{text.symptomTitle}</h1><p className="wizard-help">{text.symptomHelp}</p><select value={deviceType} onChange={(event) => { setDeviceType(event.target.value as DeviceKey); setSelected([]); setOtherSymptom(""); setCustomDevice(""); setBrand(""); setOtherBrand(""); setModel(""); }} aria-label={text.deviceLabel}>{Object.entries(devices).map(([value, labels]) => <option value={value} key={value}>{labels[language]}</option>)}</select>{deviceType==="other"&&<label className="custom-device-field">{text.customDevice}<input required maxLength={100} value={customDevice} onChange={event=>setCustomDevice(event.target.value)} placeholder={text.customDeviceExample}/></label>}<div className="symptom-list">{symptoms[deviceType].map((symptom) => <button className={selected.includes(symptom.id) ? "selected" : ""} type="button" onClick={() => toggle(symptom.id)} key={symptom.id}><i>{selected.includes(symptom.id) ? "✓" : ""}</i><span>{symptom[language]}</span></button>)}</div>{selected.includes("other")&&<label className="other-symptom-field">{text.otherLabel}<textarea required maxLength={500} value={otherSymptom} onChange={(event)=>setOtherSymptom(event.target.value)} placeholder={text.otherPlaceholder} /></label>}<button className="primary-action" type="button" disabled={!selected.length || (deviceType==="other"&&!customDevice.trim()) || (selected.includes("other") && !otherSymptom.trim())} onClick={() => setStep(2)}>{text.next}</button></section>}
        {step === 2 && <section><p className="wizard-overline">{text.step(2)}</p><h1>{text.contactTitle}</h1><div className="wizard-fields"><label>{text.brand}<select required value={brand} onChange={event=>{setBrand(event.target.value);setOtherBrand("");}}><option value="">{text.chooseBrand}</option>{brands[deviceType].map(item=><option value={item} key={item}>{item}</option>)}<option value="other">{text.otherBrand}</option></select></label>{brand==="other"&&<label>{text.otherBrandLabel}<input required maxLength={80} value={otherBrand} onChange={event=>setOtherBrand(event.target.value)}/></label>}<label>{text.model}<input required value={model} onChange={(event) => setModel(event.target.value)} placeholder={text.modelExample} /></label><label>{text.name}<input required value={name} onChange={(event) => setName(event.target.value)} /></label><label>{text.phone}<input required inputMode="tel" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="08x-xxx-xxxx" /></label><label>{text.email}<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label className="wide">{text.note}<textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder={text.noteHelp} /></label></div><fieldset className="customer-priority"><legend>{text.priority}</legend><button type="button" className={priority==="normal"?"selected":""} onClick={()=>setPriority("normal")}><i>📋</i><span><strong>{text.normal}</strong><small>{text.normalHelp}</small></span></button><button type="button" className={priority==="urgent"?"selected urgent":"urgent"} onClick={()=>setPriority("urgent")}><i>⚡</i><span><strong>{text.urgent}</strong><small>{text.urgentHelp}</small></span></button>{priority==="urgent"&&<p>ⓘ {text.priorityNotice}</p>}</fieldset><div className="wizard-actions"><button type="button" onClick={() => setStep(1)}>{text.back}</button><button className="primary-action" type="button" disabled={!deviceDetailsComplete || !name || phone.replace(/\D/g, "").length < 9} onClick={() => setStep(3)}>{text.review}</button></div></section>}
        {step === 3 && <section><p className="wizard-overline">{text.step(3)}</p><h1>{text.summaryTitle}</h1><div className="repair-summary"><div><small>{text.device}</small><strong>{resolvedTypeLabel} · {resolvedBrand} {model}</strong></div><div><small>{text.symptom}</small><strong>{summarySymptoms.join(", ")}</strong></div><div><small>{text.priority}</small><strong>{priority==="urgent"?`⚡ ${text.urgent}`:`📋 ${text.normal}`}</strong></div><div><small>{text.contact}</small><strong>{name} · {phone}</strong></div></div>{priority==="urgent"&&<p className="priority-summary-notice">ⓘ {text.priorityNotice}</p>}{error && <p className="form-error" role="alert">{error}</p>}<div className="wizard-actions"><button type="button" onClick={() => setStep(2)}>{text.edit}</button><button className="primary-action" type="submit" disabled={saving}>{saving ? text.saving : text.confirm}</button></div></section>}
      </form>
    </main>
  );
}
